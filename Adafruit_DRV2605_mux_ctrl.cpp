...
Adafruit_DRV2605::Adafruit_DRV2605(uint8_t address_in) {
	address=address_in;
}
...
uint8_t Adafruit_DRV2605::readRegister8(uint8_t reg) {
  uint8_t x ;
   // use i2c
	...
	Wire.write(0b00001011); //read from control i2c port 0
	Wire.write(DRV2605_ADDR);
    //Wire.beginTransmission(DRV2605_ADDR);
    Wire.write((byte)reg);
    Wire.endTransmission();
	//Wire.requestFrom(address);
	Wire.write(0b00001011);
	Wire.write((byte)DRV2605_ADDR);
	Wire.write((byte)1);
...
void Adafruit_DRV2605::writeRegister8(uint8_t reg, uint8_t val) {
   // use i2c
	Wire.beginTransmission(address);
	Wire.write(0b00001010); // write to control i2c port 0
    Wire.write(DRV2605_ADDR);
    Wire.write((byte)reg);
    Wire.write((byte)val);
    Wire.endTransmission();
